...
export const DEFAULT_PROVIDERS: ProviderConfig[] = [
  ...
  // --- Agent Provider Manager: the remaining free/generous providers already
  // offered for 'text' also work well for general agent tasks (they're the
  // same OpenAI-shaped chat endpoint underneath) — listed here too under
  // type: 'agents' so the Agents tab's fallback pool isn't limited to just
  // Ollama/WebLLM/Transformers.js/Puter/OpenRouter/Groq. Same adapters, same
  // conventions (disabled by default, keyed/paid opt-in) as every preset
  // above — nothing new to configure, existing API keys saved for the text
  // category are reused automatically since keys are stored per-adapter,
  // not per-provider-type.
  {
    ...
    isPreset: true,
    isBuiltIn: false,
    notes: 'Same GitHub Models endpoint as the text category (needs a PAT with the "models: read" permission), listed here too for agent tasks. ' +
      'IMPORTANT: GitHub Models is being fully retired on 2026-07-30 — see the text-category entry\'s notes for details.',
    timeoutMs: 30000,
    retries: 1,
  },
  {
    ...
    isPreset: true,
    isBuiltIn: false,
    // Same CORS gap as preset-cloudflare-text — see that entry's comment.
    requiresServerProxy: true,
