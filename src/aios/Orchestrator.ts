    ...
    try {
      ...
      switch (agent.type) {
        ...
        case 'llm': {
          ...
          const model = agent.config.model || 'openai';
          const temperature = agent.config.temperature || 0.8;
          // Use the text provider chain
          const text = await this.kernel.getProviderManager().callTextProvider(prompt, { model, temperature });
          output = text;
          break;
        }
        case 'research': {
